...
# MODIFIED VERSION. Model also returns intermediate representations before last fc layer
# Src: https://github.com/jonasricker/diffusion-model-deepfake-detection
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        # MODIFIED to also return intermediate representation
        x = self.feature(x)
        x_inter = self.avgpool(x)
        x = self.fc(x_inter)
        return x, x_inter
    ...
    def apply(self, pil):
        ...
        if (pil.size[0] > LIMIT_SIZE) and (pil.size[1] > LIMIT_SIZE):
            ...
            with torch.no_grad():
                ...
                for index0 in range(0, img.shape[-2], LIMIT_SLIDE):
                    for index1 in range(0, img.shape[-1], LIMIT_SLIDE):
                        ...
                        # MODIFIED: multiple output of model & flatten the intermediate representation
                        logit, x_inter = self(clip.to(device)[None, :, :, :])
                        x_inter = torch.flatten(x_inter).cpu().numpy()
                        logit = torch.squeeze(logit).cpu().numpy()
            ...
            with torch.no_grad():
                # MODIFIED: multiple output of model & flatten the intermediate representation
                logit, x_inter = self(self.transform(pil).to(device)[None, :, :, :])
                x_inter = torch.flatten(x_inter).cpu().numpy()
                logit = torch.squeeze(logit).cpu().numpy()

        return logit, x_inter
