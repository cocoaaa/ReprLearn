...
if __name__ == '__main__':
    ...
    parser.add_argument("--tune_metric", type=str, default="loss",
                        help="Metric Tune will use to compare hyperparameter configs. Use 'loss' for val_loss or 'mean_accuracy' for mean_val_acc")
    ...
    # if 'acc' in args.monitor_metric:
    #     tune_metric, tune_mode = 'mean_accuracy', 'max'
    # elif 'loss' in args.monitor_metric:
    #     tune_metric, tune_mode = 'loss', 'min'
    # else:
    #     raise ValueError(f"CLI arg monitor_metric must be a string containing either loss or acc: {args.monitor_metric}")

    analysis = tune.run(
        set_hparam_and_train_closure,
        config=search_space,
        metric=args.tune_metric, #'mean_accuracy' or `loss',
        mode='min' if 'loss' in args.tune_metric else 'max',
