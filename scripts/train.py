...
## Specify what metric to use for saving model checkpoints and report TB logger as the meaning of
best_score: set --monitor_metric to either `val_loss` or `val/style_acc`. These values must be used
as a logged metric's name in pytorch-lighting's DataModule definition (eg. bilatent_vae.py) using
`self.log(<monitor_metric_name>, variable_name)`
nohup python train.py --model_name="bivae" \
--latent_dim=4 --hidden_dims 32 64 64 64 --adv_dim 32  \
--data_name="multi_rotated_mnist" --angles 0 15 30 45 60 --n_styles=5 \
--gpu_id=2 --max_epochs=400   \
--monitor_metric "val/style_acc" --terminate_on_nan=True  \
-lr 3e-3 --adv_weight 3000.0 \
--log_root="/data/hayley-old/Tenanbaum2000/lightning_logs/monitor-style-acc" &
...
def train(args: Union[Dict, Namespace]):
    ...
    # monitor_metric, mode = 'val/style_acc', 'max' #todo
    ckpt_callback = ModelCheckpoint(
        monitor=args.monitor_metric,
        mode='max' if 'acc' in args.monitor_metric else 'min',
    )
    early_stopping_callback = EarlyStopping(args.monitor_metric, patience=1) #since we set Trainer's check_val_every_epoch = 10ep
    callbacks = [
        ckpt_callback,
        ...
        #         EarlyStopping(args.monitor_metric, patience=10),
    ]
    ...
    hparams['monitor_metric'] = args.monitor_metric
...
if __name__ == '__main__':
    ...
    #ModelCheckpointargs.model_ckpt_metric
    parser.add_argument("--monitor_metric", type=str, default="val_loss",
                        help="Metric pl.ModelCheckpoint Callback will use to save ckpts. Use 'val_loss' or 'val/style_acc'.")
